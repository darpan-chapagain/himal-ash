  ...
  public class RunnerSimulatePhysics3D : RunnerSimulatePhysicsBase<PhysicsScene> {
    ...
    protected override void RestoreAutoSimulate() {
      UnityEngine.Physics.simulationMode = (SimulationMode)_physicsAutoSimRestore;
        ...
        {
            // Check if the simulation mode is set to Script (which is what Fusion uses)
            get => UnityEngine.Physics.simulationMode == SimulationMode.Script;
            set
            {
                // Instead of a toggle, we manually trigger a sync if value is true
                if (value) UnityEngine.Physics.SyncTransforms();
            }
        }
        /// <inheritdoc/>
